<script lang="ts">
	...
	const navigation = [
		...
		{ name: 'Sales', href: '/manager/sales', icon: TrendingUp, color: 'from-orange-500 to-red-600' }
	];
</script>
...
>
	...
	<div class="pointer-events-none fixed inset-0 overflow-hidden">
		<div
			class="animate-blob absolute -top-40 -right-40 h-80 w-80 rounded-full bg-purple-300/30 mix-blend-multiply blur-3xl dark:bg-purple-900/20"
		></div>
		<div
			class="animate-blob animation-delay-2000 absolute -bottom-40 -left-40 h-80 w-80 rounded-full bg-cyan-300/30 mix-blend-multiply blur-3xl dark:bg-cyan-900/20"
		></div>
		<div
			class="animate-blob animation-delay-4000 absolute top-1/2 left-1/2 h-80 w-80 rounded-full bg-pink-300/30 mix-blend-multiply blur-3xl dark:bg-pink-900/20"
		></div>
	</div>
	...
	>
		...
		<div class="flex items-center gap-2">
			<div class="h-8 w-8 rounded-xl bg-gradient-to-br from-red-500 to-pink-600 p-1.5">
				<Sparkles class="h-full w-full text-white" />
			</div>
			<span
				class="bg-gradient-to-r from-red-600 to-pink-600 bg-clip-text text-lg font-bold text-transparent"
				>Kipharma</span
			>
		</div>
		<ThemeToggle />
			...
			<div class="flex h-full flex-col">
				...
				<div class="flex h-20 items-center justify-between px-6">
					<div class="flex items-center gap-3">
						<div class="relative">
							<div
								class="h-10 w-10 rounded-2xl bg-gradient-to-br from-red-500 via-pink-500 to-purple-600 p-2 shadow-lg shadow-red-500/25"
							>
								<Sparkles class="h-full w-full text-white" />
							</div>
							<div
						...
						<div>
							<span
								class="bg-gradient-to-r from-red-600 via-pink-600 to-purple-600 bg-clip-text text-xl font-bold text-transparent"
								>Kipharma</span
							>
				...
				>
					<div class="flex items-center gap-3">
						<div class="relative">
							<div
								class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 text-lg font-bold text-white shadow-lg"
							>
								{managerStore.currentManager.name.charAt(0)}
							</div>
							<div
						...
						<div class="flex-1 overflow-hidden">
							<p class="truncate text-sm font-semibold text-white">
								{managerStore.currentManager.name}
							</p>
					...
					<div class="mt-3 flex gap-2">
						<div class="flex-1 rounded-lg bg-white/10 px-2 py-1.5 text-center">
							<p class="text-xs font-bold text-white">{managerStore.myProducts.length}</p>
							<p class="text-[10px] text-slate-400">Products</p>
						</div>
						<div class="flex-1 rounded-lg bg-white/10 px-2 py-1.5 text-center">
							<p class="text-xs font-bold text-amber-400">{managerStore.lowStockItems.length}</p>
				...
				<!-- Low Stock Alert -->
				{#if managerStore.lowStockItems.length > 0}
					<div class="mx-4 mb-4">
						<div
							class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-red-500/10 p-4 ring-1 ring-amber-500/20"
						>
							<div
								class="absolute -top-4 -right-4 h-16 w-16 rounded-full bg-amber-500/20 blur-2xl"
							></div>
							<div class="relative flex items-start gap-3">
								<div
									class="rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 p-2 shadow-lg"
								>
									<Bell class="h-4 w-4 text-white" />
								</div>
								<div>
									<h4 class="text-sm font-bold text-amber-700 dark:text-amber-400">Stock Alert!</h4>
									<p class="mt-0.5 text-xs text-amber-600/80 dark:text-amber-500/80">
										{managerStore.lowStockItems.length} items need restocking
									</p>
								</div>
							</div>
						</div>
